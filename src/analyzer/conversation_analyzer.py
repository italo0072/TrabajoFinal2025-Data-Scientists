...
class ConversationAnalyzer:
    ...
    def __init__(self):
        ...
        self.sale_keywords = self.config.get('sale_keywords', [])
        self.positive_keywords = self.config.get('positive_keywords', [])
        self.negative_keywords = self.config.get('negative_keywords', [])

    def separate_speakers(self, text: str) -> List[Dict[str, Any]]:
      
        lines = [ln.strip() for ln in re.split(r"\r?\n", text) if ln.strip()]
        turns: List[Dict[str, Any]] = []

        label_pattern = re.compile(r"^(Operador|Agent|Operator|Agente|Agente:|Operador:|Cliente|Client|Customer|Cliente:)[:\-\s]*", re.IGNORECASE)
        # More precise pattern: capture optional label at start like 'Operador:' or 'Cliente -'
        label_capture = re.compile(r"^(?P<label>Operador|Agent|Operator|Agente|Cliente|Client|Customer)\s*[:\-]?\s*(?P<content>.*)$", re.IGNORECASE)

        for line in lines:
            m = label_capture.match(line)
            if m:
                raw_label = m.group('label')
                content = m.group('content').strip()
                role = 'Cliente' if raw_label.lower() in ['cliente', 'client', 'customer'] else 'Operador'
                # start a new turn
                turns.append({'role': role, 'content': content, 'timestamp': None})
            else:
                # no speaker label at start: if line contains a label elsewhere (e.g. '... Cliente: ...') try to extract
                inline = re.search(r"(Operador|Agent|Operator|Agente|Cliente|Client|Customer)\s*[:\-]\s*(.+)", line, re.IGNORECASE)
                if inline:
                    raw_label = inline.group(1)
                    content = inline.group(2).strip()
                    role = 'Cliente' if raw_label.lower() in ['cliente', 'client', 'customer'] else 'Operador'
                    turns.append({'role': role, 'content': content, 'timestamp': None})
                else:
                    # treat as continuation of the previous speaker (if exists), otherwise unknown
                    if turns:
                        # append to previous content with a space
                        if turns[-1]['content'].endswith('.') or len(turns[-1]['content']) == 0:
                            turns[-1]['content'] = turns[-1]['content'] + ' ' + line
                        else:
                            turns[-1]['content'] = turns[-1]['content'] + ' ' + line
                    else:
                        turns.append({'role': 'Unknown', 'content': line, 'timestamp': None})

        # Post-process: merge consecutive turns with same role and dedupe exact duplicate consecutive texts
        ...
        for t in turns:
            if not merged:
                ...
                continue
            prev = merged[-1]
            # if same role, append content (with guard against repeating identical lines)
            if prev['role'] == t['role']:
                # avoid exact duplicate repetition
                if t['content'].strip() == prev['content'].strip():
                    # skip exact duplicate
                    continue
                prev['content'] = (prev['content'].strip() + ' ' + t['content'].strip()).strip()
            else:
                merged.append(t)

        # Final normalization: trim whitespace
        for m in merged:
            m['content'] = re.sub(r'\s+', ' ', m['content']).strip()

        return merged

    # The rest of your class remains unchanged but we keep method names and logic the same
    def detect_sale(self, conversation: str) -> Dict[str, any]:
        ...
        return {'is_sale': is_sale, 'confidence': confidence, 'indicators_found': sale_indicators}
    ...
    def analyze_customer_satisfaction(self, turns: List[Dict]) -> Dict[str, any]:
        ...
        if not customer_turns:
            return {'satisfaction_level': 'Unknown', 'score': 0.5, 'positive_indicators': 0, 'negative_indicators': 0}
        positive_count = 0
        negative_count = 0
        ...
        score = 0.5 if total_indicators == 0 else (positive_count / total_indicators)
        if score >= 0.7:
            level = 'Satisfecho'
        elif score >= 0.4:
            level = 'Neutral'
            ...
            level = 'Insatisfecho'
        return {'satisfaction_level': level, 'score': score, 'positive_indicators': positive_count, 'negative_indicators': negative_count}
    ...
    def analyze_operator_performance(self, turns: List[Dict]) -> Dict[str, any]:
        ...
        if not operator_turns:
            return {'performance_level': 'Unknown', 'score': 0.5, 'courtesy_count': 0, 'resolution_attempt': False}
        courtesy_count = 0
        resolution_count = 0
        courtesy_keywords = self.config.get('operator_performance_metrics', {}).get('courtesy_keywords', [])
        resolution_keywords = self.config.get('operator_performance_metrics', {}).get('resolution_keywords', [])
        ...
        return {'performance_level': level, 'score': score, 'courtesy_count': courtesy_count, 'resolution_attempt': has_resolution}
    ...
    def generate_operator_feedback(self, performance: Dict, satisfaction: Dict, sale: Dict) -> str:
        feedback_parts = []
    ...
    def analyze_conversation(self, text: str) -> Dict[str, any]:
        ...
        feedback = self.generate_operator_feedback(operator_performance, customer_satisfaction, sale_info)
        return {
            'turns': turns,
            'sale_info': sale_info,
